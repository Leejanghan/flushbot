...
{  
  ...
  if (motor_num == 0) // left (want to change right)
  {
    left_motor_dir = true;
    left_motor_PID = true;
    P_GAIN_ = P_GAIN_L;
  ...
  else if (motor_num == 1) // right (want to change left)
  {
...
// 얘는 엔코더 카운터 값에 의한 각속도를 말하는거고 
// 이론적으로 엔코더 모터 값에 의한 odometry 추정이 맞는거임.
// 그걸 바탕으로 이론값과의 비교를 해야되는거고 

// d 입력시 시계 방항 회전이므로 ang_z 음수값으로 설정함 
...
// lin_x_, ang_z_ --> m/s unit
  ...
  dir_term=left_motor_dir==0?1.:-1.; // dir_term --> 1 (left motor 일때) 
...
} // 얘는 teleop 값에 의한 각속도를 말하는거임.  

// PID 제어에 대한 이해
/* target : 목표로 하는 모터의 RPM , current : 현재 모터의 RPM,
   realError : 목표 RPM과 현재 RPM 사이의 차이, accError : realErrsor의 누적 
...
uint8_t CEncoder::PIDtoPWM()
{
  ...
  pidControl_to_pwm = int((pidControl+0.58)*25./1.52);  // off pid 
  
  if (pidControl_to_pwm <= 10) pidControl_to_pwm = 0;
