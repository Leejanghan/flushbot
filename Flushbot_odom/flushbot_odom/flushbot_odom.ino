...
/* 여기서 애초에 teleop 값을 subscribe하고 이를 바탕으로 PID제어 후 PWM을 출력함. 
근데 차이가 좀 심한 걸 봤을땐, 내가 봤을 땐 일단 양쪽 보정계수를 다르게 하는 방향쪽을 생각해야할듯. 
PWM 값 출력하는 값을 한 번 봐야 알듯. */
/*******************************************************************************
...
bool calcOdometry(double diff_time)
{
  ...
  v[LEFT] = CMotor_L.ang_vel * WHEEL_RADIUS;
  w[LEFT] = CMotor_L.ang_vel;
  v[RIGHT] = CMotor_R.ang_vel * WHEEL_RADIUS;
  w[RIGHT] = CMotor_R.ang_vel;
  ...
  // 내가 생각했을 땐 wheel_l 이라는게 결국엔 ang_vel 뜻하는게 맞는거 같음.
...
void check_data()
{
  theta_msg.data = lidar_distance_filtered;
